...
public class TableViewFactory {
	 ...
	 * 
	 * @param tableView
	 *            빈껍데기(컬럼과 데이타가 없는) 테이블뷰 객체를 전달한다.
	 * @param voClazz
	 *            테이블뷰에 반영시킬 vo클래스를 지정한다.
	 */
	...
	/**
	 * VO클래스를 전달해서 테이블뷰를 리턴받는다.
	 * @param voClazz
	 * @return
	 */
	public static TableView getTable(Class voClazz) {
		TableView table = new TableView();
		setTable(table, voClazz);
		return table;
	}
	
	/**
	 * 질의와 컨넥션 객체를 받아 테이블뷰를 만들어주는 메소드 
	 * @param sql 데이타베이스 select질의를 문자열로 보낸다.
	 * @param conn 컨넥션객체를 전달한다.
	 * @return 테이블뷰 객체가 만들어져 리턴된다. 
	 */
	public static TableView getTable(String sql, Connection conn) {

		TableView<Map> table = null;
	...
	public static TableView getTable(List<Map<String, String>> data, String[] colNames) {
		Callback<TableColumn<Map, String>, TableCell<Map, String>> cellFactoryForMap = new Callback<TableColumn<Map, String>, TableCell<Map, String>>() {
			@Override
			public TableCell<Map, String> call(TableColumn<Map, String> arg0) {
